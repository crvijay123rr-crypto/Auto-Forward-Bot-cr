...
# Topic extract karne ke liye function
...
async def pub_(bot, message):
    ...
    # Task start hone se pehle database clear karein
    ...
    try: 
        ...
        await msg_edit(m, f"**Source chat may be a private channel / group. Use userbot or make bot admin.**", retry_btn(frwd_id), True)
    ...
    if locked:
        try:
            ...
            async for message in client.iter_messages(
                client, chat_id=sts.get('FROM'), limit=int(sts.get('limit')), 
                offset=int(sts.get('skip')) if sts.get('skip') else 0, continuous=is_continuous
            ):
                ...
                if forward_tag:
                    ...
                    if (len(MSG) >= 100 or (sts.get('total') - sts.get('fetched')) <= 100): 
                        ...
                        sts.add('total_files', len(MSG)); await asyncio.sleep(10); MSG = []
                else:
                    new_caption = custom_caption(message, caption)
                    details = {"msg_id": message.id, "media": media(message), "caption": new_caption, 'button': button, "protect": protect}
                    await copy(client, details, m, sts)
                    sts.add('total_files'); await asyncio.sleep(sleep) 
        ...
        if topics:
            keyboard = [[InlineKeyboardButton(f"📚 {t['name']}", url=t['link'])] for t in topics]
...
# --- BAAKI FUNCTIONS WAISE HI RAKHEN (copy, forward, edit, is_cancelled, etc.) ---
# ... (Niche baki original functions paste kar dein)
