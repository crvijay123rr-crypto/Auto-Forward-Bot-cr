...
class Database:
    ...
    def __init__(self, uri, database_name):
        ...
        self.topics = self.db.topics # New collection for Topic Summary
        
    # --- New Methods for Topic Summary ---
    
    async def add_topic(self, user_id, topic_name, link):
        # Ek naya topic add karega agar wo pehle se exist nahi karta
        await self.topics.update_one(
            {'user_id': int(user_id)}, 
            {'$addToSet': {'topics': {'name': topic_name, 'link': link}}}, 
            upsert=True
        )

    async def get_topics(self, user_id):
        data = await self.topics.find_one({'user_id': int(user_id)})
        return data.get('topics', []) if data else []

    async def clear_topics(self, user_id):
        await self.topics.delete_one({'user_id': int(user_id)})

    # --- Existing Methods (Wese hi rahengi) ---
