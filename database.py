...
def __init__(self, uri, database_name):
    ...
    self.topic = self.db.topic_summary
...
async def get_filters(self, user_id):
    filters = []
    filter_data = (await self.get_configs(user_id))['filters']
...
async def add_frwd(self, user_id):
    return await self.nfy.insert_one({
        'user_id': int(user_id)
    })
...
async def save_topic(self, user_id, channel_id, topic, first_msg_id):
    await self.topic.update_one(
        {
            "user_id": int(user_id),
...
async def get_topic(self, user_id, channel_id, topic):
    return await self.topic.find_one(
        {
            "user_id": int(user_id),
            "channel_id": int(channel_id),
            "topic": str(topic)
...
async def delete_topic(self, user_id, channel_id, topic):
    return await self.topic.delete_one(
        {
            "user_id": int(user_id),
            "channel_id": int(channel_id),
            "topic": str(topic)
...
async def get_all_topics(self, user_id, channel_id):
